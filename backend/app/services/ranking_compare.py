...
Правила (одинаковые для всех трёх сущностей):
  ...
  2) если рейтинг равен — сравниваются «медальные очки»:
     1 🥇 = 2 🥈 = 3 🥉  (6 / 3 / 2 очка);
  3) если и они равны — третичный критерий (по убыванию):
     - спортсмены: винрейт wins / (wins + losses);
     - тренеры: количество активных учеников;
     - клубы: количество активных спортсменов;
  4) если всё совпало — записи занимают одинаковое место
...
def compute_rankings(
    ...
    медальные очки, третичный критерий). Чем больше значение — тем выше
    место (reverse=True).
    """
